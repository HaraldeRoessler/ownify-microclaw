...
- Async A2A task delegation (task create / task status)
- Per-request tool allowlist for external callers
- Chat-scoped memory for external callers
...
- Live voice call integration for Matrix VoIP (m.call.* events forwarded to voice-rtc sidecar)
- Voice-call-specific agent tool restrictions (memory allowed, other tools disabled)

### Image input (multimodal)

- Matrix channel downloads m.image bytes for vision model inputs (vision-capable LLMs can now describe uploaded images)
- A2A protocol extension: `images: [{base64, mime}]` field on `A2AMessageRequest` for inbound images via the agent-api surface
...
- LLM error-message sanitization (cluster-internal hostnames stripped)

### CLI / env vars

- `MICROCLAW_SUPPRESS_WARNING_CODES` env var
- `KLAW_*` env vars renamed to `OWNIFY_*` (backwards-compat fallback preserved)

### CI / build

- Native binary builds uploaded to GitHub Releases
- GHCR image push with GHCR_PAT secret (works around fork GITHUB_TOKEN scope issue)

---

## Upstream changes (merged from microclaw/microclaw v0.2.2)
