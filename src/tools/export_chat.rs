...
impl Tool for ExportChatTool {
    ...
    async fn execute(&self, input: serde_json::Value) -> ToolResult {
        ...
        // EU AI Act Article 50: append visible AI disclosure footer
        if let Some(slug) = &self.agent_slug {
            md.push_str(&crate::ai_disclosure::disclosure_footer(
                slug,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_export_chat_appends_disclosure() {
        let (db, dir) = test_db();
        db.store_message(&StoredMessage {
            id: "m1".into(),
            chat_id: 300,
        ...
        let out_path = dir.join("export_disclosure.md");
        let tool = ExportChatTool::new(db, dir.to_str().unwrap())
            .with_agent_identity("ownify-test".into(), Some("did:moltrust:test".into()));
        let result = tool
            .execute(json!({"chat_id": 300, "path": out_path.to_str().unwrap()}))
            .await;
        assert!(!result.is_error, "{}", result.content);
        ...
        assert!(content.contains("ownify-test"));
        assert!(content.contains("did:moltrust:test"));
        cleanup(&dir);
    }
}
